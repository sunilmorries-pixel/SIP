...
`zoho_data`, no polygon inside a raster basemap tile, and no state-centroid table for the map's
state-tier layer to draw circles at. They are edited in place, and every one
...
| `INDIA_STATE_CENTROIDS_` | `src/client/MapView.html` | **36** rows (all 28 states + 8 union territories) | **not** third-party data — hand-authored approximate centroids, public-domain administrative geography, not extracted from any licensed dataset | edited in place; add a row for any state name the data can produce | the state-tier proportional-circle layer between zoom 5–6 on all three maps (`renderStateLayer_`, added @98–@103) | Matching is **exact-name** against each center row's own `state` field — same trust-the-data convention `countryHasCenter_` already uses for `COUNTRY_GEOJSON_`'s `name` property. A row whose state isn't one of these 36 names (typo, blank, non-India) silently contributes to no circle |
