...
function buildDashboardQuerySpecs(hub, filters) {
  ...
  return [
    ...
    {
      // Same-MONTH resolution: of the tickets OPENED so far THIS calendar
      // month, what share were ALSO closed within this same month. A
      // same-cohort completion rate — different question from zohoTrend
      // above, whose created/closed counts mix cohorts (a ticket counted as
      // "closed" in March may have opened in January). Scoped to the current
      // month only (was a 12-month trend chart; per user, 2026-09-04, now a
      // single current-month KPI — see kpiZResolution). completion_pct
      // divides by opened, NULLIF-guarded against a zero-ticket month.
      key: 'zohoMonthlyCompletion',
      params: p,
      sql:
        "WITH t AS (SELECT " + zohoParsedDates_() + " FROM " + zohoDedupSql_() + " WHERE " + HUB_FILTER_SQL + centerCond + supportDateCond +
        " AND FORMAT_DATETIME('%Y-%m', CreatedAt) = FORMAT_DATETIME('%Y-%m', CURRENT_DATETIME())) " +
